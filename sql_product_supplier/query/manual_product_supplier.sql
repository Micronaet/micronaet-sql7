SELECT clienti.CDS_CNT, gruppo.* FROM `dbmirror`.`PA_RUBR_PDC_CLFR` clienti JOIN (
    SELECT CKY_CNT_CLFR, CKY_ART, max(DTT_SCAD) FROM (
        ...
        UNION
        SELECT CKY_CNT_CLFR, CKY_ART, DTT_SCAD FROM `dbmirror18`.`MM_RIGHE` WHERE CSG_DOC = 'BF' 
        UNION
        SELECT CKY_CNT_CLFR, CKY_ART, DTT_SCAD FROM `dbmirror17`.`MM_RIGHE` WHERE CSG_DOC = 'BF' 
        UNION
